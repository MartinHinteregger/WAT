...
LMS7_Device* LMS7_Device::CreateDevice(const lime::ConnectionHandle& handle, LMS7_Device *obj)
{
    ...
    // WAT: Deactivated all SDR Versions excep the standard one.
    ...
    if (info.deviceName != lime::GetDeviceName(lime::LMS_DEV_UNKNOWN))
        device = new LMS7_LimeSDR(conn,obj);
    else
    {
        printConsoleAndDebugLine("Attention: Only standard LimeSDR are supported in this software. Others, like mini, were deactivated.");
