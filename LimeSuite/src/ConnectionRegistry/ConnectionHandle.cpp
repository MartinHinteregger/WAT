...
std::string ConnectionHandle::serialize(void) const
{
    ...
    // WAT: addded custom identifiers
    if ((not addr.empty()) & !serial.find("0009070602461225"))
    	out += ", YELLOW";
    else if ((not addr.empty()) & !serial.find("0009070105C50D11"))
    	out += ", RED";
    else if ((not addr.empty()) & !serial.find("0009070602451726"))
    	out += ", GREEN";
    else if ((not addr.empty()) & !serial.find("0009072C02881C16"))
    	out += ", BLUE";
    else
    	out += ", noname";
