...
void Streamer::AlignRxTSP()
{
    ...
    //alignment search
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            ...
            {
                lime::warning("AlignRxTSP: Channel alignment failed");
                break;
            }
            fpga->StopStreaming();
...
double Streamer::GetPhaseOffset(int bin)
{
    ...
    if (dataPort->ReceiveData((char*)buf, sizeof(FPGA_DataPacket), chipId, 50)!=sizeof(FPGA_DataPacket))
    {
        lime::warning("GetPhaseOffset: Channel alignment failed");
        delete [] buf;
        return -1000;
    }
    fpga->StopStreaming();
...
void Streamer::AlignRxRF(bool restoreValues)
{
    ...
    if (found)
        ...
        lime::warning("AlignRxRF: Channel alignment failed");
...
void Streamer::AlignQuadrature(bool restoreValues)
{
    ...
    if (restoreValues)
        lms->RestoreRegisterMap(regBackup);
    if (!found)
        lime::warning("AlignQuadrature:Channel alignment failed");
}
